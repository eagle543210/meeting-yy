...
class ConnectionManager:
    ...
    async def disconnect(self, client_id: str, meeting_id: Optional[str] = None):
        """
        处理 WebSocket 连接断开。
        ...
        # 从 active_connections 中获取并移除连接信息
        connection_info = self.active_connections.pop(client_id, None)
        if connection_info:
            disconnected_websocket, actual_meeting_id = connection_info
            ...
            # 更新系统统计
            self.system_stats["active_connections"] = len(self.active_connections)
            # total_connections 不会减少，因为它代表历史总数
            # 重新计算活跃会议数
            self.system_stats["meetings_active"] = len(set(mid for _, mid in self.active_connections.values()))
    ...
    async def send_personal_message(self, message: Dict[str, Any], client_id: str):
        ...
        if connection_info:
            ...
            try:
                ...
                await self.disconnect(client_id, meeting_id) # 使用正确的 meeting_id 进行清理
    ...
    async def send_message_to_meeting(self, message: Dict[str, Any], meeting_id: str):
        ...
        for client_id in clients_in_meeting:
            ...
            if connection_info:
                ...
                disconnected_clients.append(client_id) # 标记为断开

        # 移除已断开的客户端
        for client_id_to_disconnect in disconnected_clients:
            await self.disconnect(client_id_to_disconnect, meeting_id)
    ...
    async def broadcast(self, message: Dict[str, Any]):
        ...
        通常用于系统状态更新等。
        """
        disconnected_clients_info = [] # 存储 (client_id, meeting_id)
        ...
        for client_id, (websocket, meeting_id) in list(self.active_connections.items()):
            try:
                ...
                disconnected_clients_info.append((client_id, meeting_id))
            except Exception as e:
                logger.error(f"向客户端 {client_id} (会议 {meeting_id}) 广播消息失败: {e}", exc_info=True)
                disconnected_clients_info.append((client_id, meeting_id))

        # 移除已断开的客户端
        for client_id, meeting_id in disconnected_clients_info:
            await self.disconnect(client_id, meeting_id)
