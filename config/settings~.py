...
class Settings(BaseSettings):
    ...
    USE_CUDA: bool = os.getenv("USE_CUDA", "True").lower() == "true"  # 是否尝试使用 CUDA (GPU) 加载模型
    ...
    VAD_SPEECH_THRESHOLD: float = float(os.getenv("VAD_SPEECH_THRESHOLD", 0.5)) # 语音活动检测的阈值
    MIN_SPEECH_DURATION_OFF: float = 1.0  #语音停顿的最小持续时间
    PYANNOTE_MIN_SPEECH_DURATION_S: float = 0.5 # Pyannote 说话人分离模型的最小语音段持续时间 (秒)
    ...
    # 音频能量阈值，低于此值视为静音
    AUDIO_ENERGY_THRESHOLD: float = float(os.getenv("AUDIO_ENERGY_THRESHOLD", 0.1))
    ...
    STT_TEMPERATURE: float = float(os.getenv("STT_TEMPERATURE", 0.2))
    ...
    LLM_MODEL_NAME: str = os.getenv("LLM_MODEL_NAME", "internlm2_5-1_8b-chat-q8_0.gguf") 
    LLM_MODEL_PATH: Path = Path(os.path.join(MODEL_BASE_DIR, "gguf", LLM_MODEL_NAME)) 
    # llama_cpp 相关的配置参数
  
    LLAMA_N_GPU_LAYERS: int = int(os.getenv("LLAMA_N_GPU_LAYERS", 10000)) 
