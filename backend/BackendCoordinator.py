...
class BackendCoordinator:
   ...
   """
    def __init__(self,
        ...
        # LLM 并发控制信号量，在单机、单用户场景下设置为 1，以实现串行执行
        # 优化建议:
        # 1. 这里的并发信号量被硬编码为1，对于单用户场景很合适，但在多用户或并行任务场景下可能会成为瓶颈。
        # 2. 将此值作为配置项（例如：self.settings.LLM_CONCURRENCY_LIMIT），以便根据部署环境进行调整。
    ...
    async def _get_documents_by_speaker_id(self, speaker_id: str) -> list:
        ...
        try:
            # 使用 Milvus 服务的 query 方法，过滤 speaker_id
            query_expr = f"speaker_id == '{speaker_id}'"
            result_docs = await self.milvus_service.query_collection(
    ...
    async def get_answer_from_llm(self, question: str) -> AsyncIterator[str]:
        ...
        try:
            async with self._llm_semaphore:
                # 注意这里只传入了 'prompt' 参数
                async for chunk in self.llm_model.generate_text(prompt=full_prompt, stream=True):
                    yield chunk
        except Exception as e:
