// client-app\src\App.jsx (或 App.js)
...
// --- 后端 API 配置 ---
// API 基础 URL，假定后端与前端在同一域名和端口
// 在 Vite 开发模式下，前端会通过 Vite 的开发服务器运行，
// 因此 API 请求需要指向 FastAPI 的端口 (例如 8000)。
// 在生产构建后，Vite 会将此替换为相对路径。
...
function App() {
    ...
    const [activeSpeakers, setActiveSpeakers] = useState(new Set()); // 使用 Set 优化活跃发言人管理
    ...
    wsRef.current = ws; // 每次 ws 状态更新时，同步更新 ref
    const mediaStreamRef = useRef(null);

    // --- API 调用函数 (定义在其他函数依赖它们之前) ---
    // 获取所有用户列表
    ...
    const fetchSystemStatus = useCallback(async () => {
        try {
            ...
            const data = await response.json();
            setSystemStatus(data); // 更新系统状态
            // console.log('系统状态:', data); // 调试日志
        } catch (error) {
    ...
    // --- WebSocket 连接管理 (依赖 fetchUsers，所以定义在 fetchUsers 之后) ---
    const connectWebSocket = useCallback(() => {
        // 如果 WebSocket 已打开，则不重复连接
        ...
        console.log(`尝试连接 WebSocket 到 ${wsUrl}`);
        // 创建新的 WebSocket 实例
        ...
        // WebSocket 接收消息事件
        newWs.onmessage = async (event) => { // 将 onmessage 设置为 async 函数
            ...
            if (messageData instanceof Blob) {
                ...
                // 如果接收到的是 ArrayBuffer，这通常是音频数据。
                // 打印警告并忽略，因为我们期望的是 JSON 文本消息。
                console.warn("Received ArrayBuffer from WebSocket. This is unexpected for JSON messages. Ignoring.");
                return; // 忽略二进制数据，不尝试解析为 JSON
            }

            // 现在，messageData 应该是一个字符串（要么是原始字符串，要么是 Blob 转换后的字符串）
                ...
                try {
                    ...
                    switch (message.type) {
                        ...
                        case 'transcript_update':
                            ...
                            // setActiveSpeakers(prev => new Set(prev).add(message.speakerId)); // 移到单独的 useEffect 处理
                            break;
    ...
    // 如果正在录音或已静音，则不执行
    if (isRecording || isMicMuted) return;
    ...
    try {
        ...
        const originalSampleRate = audioCtx.sampleRate; // 获取原始采样率

        // 创建媒体流源节点
        const source = audioCtx.createMediaStreamSource(stream);
        setMediaStreamSource(source);

        // 创建音频分析器节点，用于获取音频数据进行可视化
        const analyser = audioCtx.createAnalyser();
        analyser.fftSize = 256; // 设置 FFT 大小
        setAnalyserNode(analyser);
        
        // --- 动态创建 AudioWorklet 模块的代码开始 ---
    ...
    return (
        <AppContext.Provider value={appContextValue}>
            {/* 添加一个背景色和文本颜色，以便立即看到 Tailwind 是否工作 */}
...
function MicrophoneMonitor() {
    ...
    useEffect(() => {
        ...
        // **修改这里：** 从左上角 (X=0, Y=0) 开始填充，宽度为 barWidth，高度为整个 canvas 高度
    ...
    return (
        <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700 flex flex-col items-center">
            ...
            <div className="w-full h-8 bg-gray-900 rounded-lg mt-4 overflow-hidden relative border border-gray-700">
                ...
                <div 
                    // **修改这里：** 使用 top-0 和 left-0 定位，宽度动态变化，高度 100% 充满父容器
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto custom-scrollbar p-2 mb-4 bg-gray-900 rounded-lg border border-gray-700">
                ...
                {llmQAHistory.map((msg, index) => (
                    <div key={index} className={`mb-2 p-2 rounded-lg ${msg.type === 'user' ? 'bg-blue-700 text-white ml-auto max-w-[80%]' : 'bg-gray-700 text-gray-100 mr-auto max-w-[80%]'}`}>
                        <p className="font-semibold text-xs mb-1">{msg.type === 'user' ? '您' : 'AI 助手'}</p>
